...
def get_single_animal(id):
    """
    THIS FUNCTION GETS ONE ANIMAL BY ITS ID
    """
    with sqlite3.connect("./kennel.sqlite3") as conn:
        conn.row_factory = sqlite3.Row
        db_cursor = conn.cursor()
...
def get_animals_by_location(location_id):
    """GET ALL ANIMALS FOR A SINGLE LOCATION
    """
    with sqlite3.connect("./kennel.sqlite3") as conn:
        conn.row_factory = sqlite3.Row
        db_cursor = conn.cursor()

        # Write the SQL query to get the animal information you want
        db_cursor.execute("""
        select
            a.id,
            a.name,
...
def get_animals_by_status(status):
    """GET ALL ANIMALS WITH A CERTAIN STATUS
    """
    with sqlite3.connect("./kennel.sqlite3") as conn:
        conn.row_factory = sqlite3.Row
        db_cursor = conn.cursor()
...
def create_animal(new_animal):
    """GO ON NOW, MAKE AN ANIMAL W/ SQL
    """
    with sqlite3.connect("./kennel.sqlite3") as conn:
        db_cursor = conn.cursor()
...
def update_animal(id, new_animal):
    """update an animals info, see if any animals match the id that is provided to
    the method.
    """
    with sqlite3.connect("./kennel.sqlite3") as conn:
        db_cursor = conn.cursor()
